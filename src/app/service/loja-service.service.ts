...
export class LojaServiceService {
  ...
  constructor(private fileSystemService: FileSystemServiceService) { 
    this.fileSystemService.ler().then(data => {this.produtos = data; this.produtos.concat([
    ...
    this.promoProdutos = this.obterPromoProdutos();
  }

  obterPromoProdutos(): Produto[] {
    const produtoPromo = this.produtos.filter(element => element.promocao === true)
    return produtoPromo;
  }
